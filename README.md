...
- 支持单选、多选、判断、填空和简答，以及章节练习、错题复习、掌握度和间隔复习。
...
数据库、题目图片、导入原件和配置都保存在 `data` 目录。备份或恢复前先停止软件，再完整复制或替换该目录。归档项目不会删除内容；永久删除项目和非空知识节点前，软件会自动创建备份。
...
支持 `STUDY_DATA_DIR`、`STUDY_DB_NAME`、`STUDY_SECRET` 和 `STUDY_MAX_UPLOAD_MB`。旧环境变量 `H3CSE_DATA_DIR`、`H3CSE_SECRET` 继续兼容。
