    ...
    with db() as conn:
        ...
        if request.method == "POST":
            ...
            if not name:
                ...
                try:
                    conn.execute("INSERT INTO subjects(project_id,name,code) VALUES (?,?,?)", (project_id, name, request.form.get("exam_code", "").strip()))
                ...
                conn.execute("INSERT INTO chapters(subject_id,name,is_core) VALUES (?,?,?)", (request.form["subject_id"], name, int("is_core" in request.form)))
            elif kind == "point":
                if not id_belongs_to_project(conn, "chapter", request.form["chapter_id"], project_id): abort(404)
                conn.execute("INSERT INTO knowledge_points(chapter_id,name) VALUES (?,?)", (request.form["chapter_id"], name))
        ...
        chapters = conn.execute("SELECT c.*,s.name subject_name FROM chapters c JOIN subjects s ON s.id=c.subject_id WHERE s.project_id=? ORDER BY s.id,c.id", (project_id,)).fetchall()
        points = conn.execute("""SELECT kp.*,c.name chapter_name FROM knowledge_points kp JOIN chapters c ON c.id=kp.chapter_id
...
def knowledge_rename():
    ...
    with db() as conn:
        ...
        if kind == "subject":
            ...
            conn.execute(f"UPDATE {allowed[kind]} SET name=? WHERE id=?", (name, request.form["id"]))
    flash("名称已更新", "success")
    return redirect(url_for("knowledge"))
...
def questions():
    with db() as conn:
        ...
        for key, column in (("status", "q.status"), ("type", "q.type"), ("subject_id", "s.id"), ("chapter_id", "c.id")):
